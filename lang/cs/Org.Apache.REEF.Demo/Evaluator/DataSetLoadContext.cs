...
namespace Org.Apache.REEF.Demo.Evaluator
{
    ...
    internal sealed class DataSetLoadContext : IObserver<IContextStart>
    {
        ...
        [Inject]
        private DataSetLoadContext([Parameter(typeof(SerializedInitialDataLoadPartitions))] ISet<string> serializedInitialDataLoadPartitions,
        ...
        {
            foreach (var serializedInitialDataLoadPartition in serializedInitialDataLoadPartitions)
            {
                var forkedInjector =
                    injector.ForkInjector(avroConfigurationSerializer.FromString(serializedInitialDataLoadPartition));
                var inputPartition = forkedInjector.GetInstance<IInputPartition<string[]>>();
