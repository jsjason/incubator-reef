...
namespace Org.Apache.REEF.Demo.Evaluator
{
    public sealed class DataSetManager : IDataSetManager
    {
        ...
        public IEnumerable<IInputPartition<T>> GetLocalPartitions<T>(string dataSetId)
        {
            return _localPartitions[dataSetId].Select(partition => (IInputPartition<T>)partition);
        }
    }
}
