    ...
    public sealed class DataSet<T> : IDataSet<T>
    {
        ...
        public IDataSet<T2> TransformPartitions<T2>(IConfiguration transformConf)
        {
            ...
            IConfiguration stageConf = StageDriverConfiguration.ConfigurationModule
                ...
                .Set(StageDriverConfiguration.OnTaskCompleted, GenericType<TransformStage<T, T2>>.Class)
                .Build();

            return RunStage<T2>(Configurations.Merge(serializedTransformConf, stageConf));
        }

        public IDataSet<T2> TransformPartitions<T2, TTransform>() where TTransform : ITransform<T, T2>
        {
            IConfiguration transformConf = TangFactory.GetTang().NewConfigurationBuilder()
                .BindImplementation(GenericType<ITransform<T, T2>>.Class, GenericType<TTransform>.Class)
                .Build();

            return TransformPartitions<T2>(transformConf);
        }
        ...
        public IEnumerable<T> Collect()
        {
            ...
            foreach (var partitionInfo in _dataSetInfo.PartitionInfos)
            {
                partitionInfo.LoadedContexts.ForEach(context =>
                {
